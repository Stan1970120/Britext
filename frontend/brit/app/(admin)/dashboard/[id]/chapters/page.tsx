...
export default function ChapterEditorPage() {
  ...
  const fetchChapters = async () => {
    try {
      const res = await fetch(API.GET_CHAPTERS(id as string), { credentials: "include" });
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API.ADD_CHAPTER(id as string)}/${selectedChapter._id}`, {
        method: "PUT",
  ...
  const addNewChapter = async () => {
    ...
    try {
      const res = await fetch(API.ADD_CHAPTER(id as string), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="h-screen flex flex-col bg-white">
      ...
      <div className="border-b px-6 py-4 flex justify-between items-center bg-gray-50">
        ...
        <div className="flex gap-3">
          <Link href={`/preview/${id}`} className="px-4 py-2 border rounded-md text-sm font-medium">Preview</Link>
          <button 
            onClick={handleSave}
            disabled={saving}
          ...
          {selectedChapter ? (
            <div className="max-w-3xl mx-auto w-full space-y-6">
              ...
              <textarea
                ...
                className="w-full h-[60vh] text-lg font-serif leading-relaxed outline-none resize-none"
