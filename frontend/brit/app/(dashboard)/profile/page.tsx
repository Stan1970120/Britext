...
export default function UserDashboard() {
  ...
  // Helper to resolve user identifier safely
  const userId = user?.id || user?._id;

  // Fetch My Books with resilient endpoint fallback
  useEffect(() => {
    const fetchMyBooks = async () => {
      ...
      try {
        ...
        // Primary Attempt: /api/me/books or API.MY_BOOKS
        const primaryUrl = API?.MY_BOOKS || "/api/me/books";
        ...
        // Fallback Attempt if primary endpoint returns 404
        if (response.status === 404 && userId) {
          const fallbackUrl = `/api/users/${userId}/books`;
            ...
            <div className="lg:col-span-3 space-y-2 animate-in fade-in slide-in-from-left-4 duration-300">
              {(["my-books", "store", "cart", "saved", "history"] as Tab[]).map((tab) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-6 py-4 rounded-xl font-bold transition-all capitalize ${
                    activeTab === tab 
                ...
                >
                  ...
                  {/* Cart Item Badge Indicator */}
                  {tab === "cart" && totalCartCount > 0 && (
                    <span 
                      className={`text-xs font-extrabold px-2.5 py-0.5 rounded-full transition-colors ${
                        activeTab === "cart"
                          ? "bg-white text-[#005F7A]"
                          : "bg-[#005F7A] text-white"
                      }`}
                    >
                      {totalCartCount}
                    </span>
...
export default function UserDashboard() {
  ...
  // Fetch My Books using authenticated API endpoint
  useEffect(() => {
    const fetchMyBooks = async () => {
      if (!token) return;
      ...
      try {
        setFetchingBooks(true);
        const response = await fetch(API.MY_BOOKS, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
                  ...
                  {loadingCart ? (
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="border-t border-slate-100 pt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        <Link 
                          href="/checkout"
                          className="w-full sm:w-auto bg-[#005F7A] text-[#ffffff] px-8 py-3.5 rounded-xl font-bold hover:opacity-90 transition-opacity text-center shadow-lg shadow-sky-50"
                        >
                          Proceed to Checkout
                        </Link>
                      </div>
