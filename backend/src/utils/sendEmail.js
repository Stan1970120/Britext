...
export const sendEmail = async (to, subject, htmlContent) => {
  try {
    ...
    console.error("Resend API Email Delivery Failure:", errorDetails);
    throw new Error(`Resend Delivery Failed: ${JSON.stringify(errorDetails)}`);
  }
};

export const sendOtpEmail = async (to, otp) => {
  const subject = "Your Verification Code";
  // Fixed invalid CSS property 'rounded-radius' -> 'border-radius'
  const htmlContent = `
